...
def mSDA(data, prob_corruption, num_layers):
...
def mSDA_lowDimApprox(data, prob_corruption, num_layers, max_subproblem_dimensionality):
	...
	while num_featuresConsidered + subproblem_dim <= dimensionality: #make sure we don't overshoot
		...
		#learn a mapping for this subspace, and save the data and mapping
	...
	#QUESTION: will we apply tanh() twice to the first layer, since we apply it once in mDA and once here?
	firstLayer_output = np.tanh(avg_of_reconstructions)
